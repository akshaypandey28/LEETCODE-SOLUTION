    ...
    int shortestPathBinaryMatrix(vector<vector<int>>& grid) {
        ...
        while (!pq.empty()) {
            ...
            for (auto dir : directions) {
                int x_ = x + dir[0];
                int y_ = y + dir[1];
                int dist = 1; //from curr_node to neighbouring node distance is 1 and add it to
                //source to curr_node distance 

                //overall src to curr_node + curr_node to neighbouring node
